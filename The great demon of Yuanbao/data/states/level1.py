...
class Level1(tools._State):
    ...
    def startup(self, current_time, persist):
        ...
        # 将参数current_time赋给字典self.game_info中'current time'键指向的值
        self.game_info[c.CURRENT_TIME] = current_time
        # 将字符串'not frozen'赋给字典self.game_info中'level state'键指向的值
        ...
        # 传入参数self.game_info 和 c.LEVEL（即：'level'）给info.OverheadInfo类
        # 实例化self.overhead_info_display
        self.overhead_info_display = info.OverheadInfo(self.game_info, c.LEVEL)
        # 传入参数self.overhead_info_display给game_sound.Sound类
